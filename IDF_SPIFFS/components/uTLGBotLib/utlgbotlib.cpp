...
/* Macros */
    ...
    #define F(x) x
    #define PSTR(x) x
	#define snprintf_P(str, n, ...) do { sprintf(str, __VA_ARGS__); } while(0)
...
uTLGBot::uTLGBot(Tag* token, const bool dont_keep_connection)
{
	...
	for(int i=0;;i++){
		//c =
		_printf("%c", (char)tlg_api_ca_pem_start[i]);
		if(tlg_api_ca_pem_start[i] == 0) break;
	}
#endif
    ...
    memset(_json_elements, 0, MAX_JSON_ELEMENTS*sizeof(jsmntok_t));
    memset(_json_subelements, 0, MAX_JSON_SUBELEMENTS*sizeof(jsmntok_t));
...
{
    _println(F("Connecting to telegram server..."));
    _printf("Heap before connecting %d\n", esp_get_free_heap_size());

    if(is_connected())
    {
        _println(F("Already connected to server."));
        return true;
    }
    ...
    if(conn_res != 1)
    {
        _printfW(F("Conection fail."));
        return false;
    }

    _println(F("Successfully connected."));
    _printf("Heap after connecting to server %d\n", esp_get_free_heap_size());
    return true;
}
...
{
    _println(F("Disconnecting from telegram server..."));
    _printf("Heap before deleting %d\n", esp_get_free_heap_size());
    if(!is_connected())
    {
        _println(F("Already disconnected from server."));
        return;
    }
    _client->disconnect();

    _println(F("Successfully disconnected."));
    _printf("Heap after deleting %d\n", esp_get_free_heap_size());
}
    ...
    bool disable_web_page_preview, bool disable_notification, uint64_t reply_to_message_id)
{
    ...
    if(parse_mode[0] != '\0')
    {
        ...
        if((strcmp(parse_mode, "Markdown") == 0) || (strcmp(parse_mode, "HTML") == 0))
        {
            ...
            msg[strlen(msg)-1] = '\0';
#if 0
            snprintf_P(temp, HTTP_MAX_BODY_LENGTH, PSTR("%s, \"parse_mode\":\"%s\"}"), msg,parse_mode);
            snprintf_P(msg, HTTP_MAX_BODY_LENGTH, PSTR("%s"), temp);
#endif

            strcat(msg, "\"parse_mode\":\"");
			strcat(msg,parse_mode);
			strcat(msg,"\"}");
        }
        else
            _printfE("Warning: Invalid parse_mode provided.");
    }
    ...
    if(disable_web_page_preview)
    {
        ...
        snprintf_P(msg, HTTP_MAX_BODY_LENGTH, PSTR("%s"), temp);
#endif
        strcat(msg, ", \"disable_web_page_preview\":true}");
    }
    ...
    if(disable_notification)
    {
        ...
        snprintf_P(msg, HTTP_MAX_BODY_LENGTH, PSTR("%s"), temp);
#endif
        strcat(msg,", \"disable_notification\":true}");
    }
    ...
    if(reply_to_message_id != 0)
    {
        msg[strlen(msg)-1] = '\0';
#if 0
        snprintf_P(temp, HTTP_MAX_BODY_LENGTH, PSTR("%s, \"reply_to_message_id\":%" PRIu64 "}"),msg, reply_to_message_id);
        snprintf_P(msg, HTTP_MAX_BODY_LENGTH, PSTR("%s"), temp);
#endif
        strcat(msg,", \"reply_to_message_id\":");
        snprintf_P(msg, HTTP_MAX_BODY_LENGTH, PSTR("%" PRIu64 ""),reply_to_message_id);
        strcat(msg,"}");
    }

    // Send the request
    _println(F("Trying to send message request..."));
...
uint8_t uTLGBot::getUpdates(void)
{
    ...
    //static char msg[HTTP_MAX_BODY_LENGTH];
    snprintf_P(msg, HTTP_MAX_BODY_LENGTH, PSTR("{\"offset\":%d, \"limit\":1, " \
        "\"timeout\":%d, \"allowed_updates\":[\"message\"]}"), (int)_last_received_msg,
        (int)TELEGRAM_LONG_POLL);

    // Send the request
    _println(F("Trying to send getUpdates request..."));
    _printf("%s get Update [size: %d]  Message %s\r\n" ,getTime(), strlen(msg), msg);
    ...
    if(request_result == false)
    {
        _printfW(F("Command fail, no response received."));
    ...
    {
        _println(F("There is not new message."));
    ...
    if(num_elements == 0)
    {
        _printfE(F("Error: Bad JSON sintax from received response."));
    ...
    if(key_position != 0)
    {
        ...
        if(num_subelements == 0)
            _printfE(F("Error: Bad JSON sintax in \"from\" element."));
        else
        {
...
#endif
                strcpy(received_msg.from.id,_json_subvalue_str);
            }
            ...
            if(key_position != 0)
            {
                ...
                // Save value in variable
#if 0
                snprintf_P(received_msg.from.first_name, MAX_USER_LENGTH, PSTR("%s"), _json_subvalue_str);
#endif
                strcpy(received_msg.from.first_name,_json_subvalue_str);
            ...
            if(key_position != 0)
            {
                ...
                // Save value in variable
#if 0
                snprintf_P(received_msg.from.last_name, MAX_USER_LENGTH, PSTR("%s"), _json_subvalue_str);
#endif
                strcpy(received_msg.from.last_name,_json_subvalue_str);
            }
            ...
            if(key_position != 0)
            {
                ...
                // Save value in variable
#if 0
                snprintf_P(received_msg.from.username, MAX_USERNAME_LENGTH, PSTR("@%s"), _json_subvalue_str);
#endif
                strcat(received_msg.from.username,"@");
                strcat(received_msg.from.username,_json_subvalue_str);
            }
            ...
            if(key_position != 0)
            {
                ...
                // Save value in variable
#if 0
                snprintf_P(received_msg.from.language_code, MAX_LANGUAGE_CODE_LENGTH, PSTR("%s"), _json_subvalue_str);
#endif
                strcpy(received_msg.from.language_code,_json_subvalue_str);
            }
        }
    }
    ...
    if(key_position != 0)
    {
        ...
        else
        {
            ...
            if(key_position != 0)
            {
                ...
                // Save value in variable
#if 0
                snprintf_P(received_msg.chat.id, MAX_ID_LENGTH, PSTR("%s"), _json_subvalue_str);
#endif
                strcpy(received_msg.chat.id,_json_subvalue_str);
            ...
            if(key_position != 0)
            {
                ...
                // Save value in variable
#if 0
                snprintf_P(received_msg.chat.type, MAX_CHAT_TYPE_LENGTH, PSTR("%s"),  _json_subvalue_str);
#endif
                strcpy(received_msg.chat.type,_json_subvalue_str);
            }
            ...
            if(key_position != 0)
            {
                ...
                // Save value in variable
#if 0
                snprintf_P(received_msg.chat.title, MAX_CHAT_TITLE_LENGTH, PSTR("%s"), _json_subvalue_str);
#endif
                strcpy(received_msg.chat.title,_json_subvalue_str);
            ...
            if(key_position != 0)
            {
                ...
                // Save value in variable
#if 0
               snprintf_P(received_msg.chat.username, MAX_USERNAME_LENGTH, PSTR("%s"),  _json_subvalue_str);
#endif
               strcpy(received_msg.chat.username,_json_subvalue_str);
            }
            ...
            if(key_position != 0)
            {
                ...
                // Save value in variable
#if 0
                snprintf_P(received_msg.chat.first_name, MAX_USER_LENGTH, PSTR("%s"), _json_subvalue_str);
#endif
                strcpy(received_msg.chat.first_name,_json_subvalue_str);
            }
            ...
            if(key_position != 0)
            {
                ...
                // Save value in variable
#if 0
                snprintf_P(received_msg.chat.last_name, MAX_USER_LENGTH, PSTR("%s"),  _json_subvalue_str);
#endif
            }
    ...
    const unsigned long response_timeout)
{
    ...
    if(pos == -1)
    {
        ...
        _printfW(F("Unexpected response."));
    ...
    char* response, const size_t response_len, const unsigned long response_timeout)
{
    ...
    {
        // Clear response if unexpected response
        _printfE(F("Unexpected response."));
        memset(response, '\0', response_len);
        return false;
    }
    response = response + pos;
    ...
    jsmntok_t* json_tokens, const uint32_t json_tokens_len)
{
    ...
    if(num_elements < 0)
    {
        _printfE(F("Can't parse JSON data. Code %d"), num_elements);
        return 0;
    }
    if((num_elements == 0) || (json_tokens[0].type != JSMN_OBJECT))
    {
        _printfE(F("Can't parse JSON data (invalid sintax?)."));
        return 0;
    }
    ...
    const uint32_t num_tokens, char* converted_str, const uint32_t converted_str_len)
{
    ...
    if(key_position == 0)
    {
        _printfW(F("No key found inside json."));
        return false;
    }
    else
    {
